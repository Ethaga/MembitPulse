...
export default function FlowiseChat({ meta }: { meta?: any }) {
  ...
  async function send(msg: string) {
    ...
    if (configOk === false) {
      setMessages((m) => [
        ...m,
        { role: "assistant", text: "FLOWISE_API_URL not configured" },
      ]);
      return;
    }
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        setMessages((m) => [
          ...
          {
            role: "assistant",
            text: "⚠️ Connection error, check Flowise config",
          },
        ]);
        return;
      }
      ...
      if (msgText.includes("body stream") || msgText.includes("already read")) {
        ...
        setMessages((m) => [
          ...m,
          { role: "assistant", text: `Error: ${msgText}` },
        ]);
      }
    } finally {
      setLoading(false);
  ...
  return (
    <div className="mt-3 text-xs text-foreground/60">
      {configOk === false && (
        <div className="p-3 rounded-md bg-[#1b0f0f] border border-red-600/30 text-sm">
      ...
      <div
        ref={scroller}
        className="border rounded-md bg-[#070707] p-3 max-h-56 overflow-auto text-xs space-y-3 neon-border"
      >
        {messages.map((m, i) => (
          <div
            key={i}
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button
          disabled={loading}
          onClick={() => send(input)}
          className="px-4 py-2 rounded-md bg-cyan-600/10 border border-cyan-400/30 text-sm"
        >
          Send
        </button>
      </div>
