  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        else if (json.mcp && Array.isArray(json.mcp.posts))
          extracted = json.mcp.posts;
        else if (json.mcp && Array.isArray(json.mcp.items))
          extracted = json.mcp.items;
  ...
  return (
    <div className="rounded-2xl border border-cyan-400/30 bg-[#0f0f0f]/80 neon-border p-4">
      ...
      <div className="mt-3 space-y-3">
        {loading && (
          <div className="text-xs text-foreground/60">Loading posts…</div>
        )}
        {error && <div className="text-xs text-secondary">{error}</div>}

        {!loading && !error && posts.length === 0 && (
          <div className="text-xs text-foreground/60">
            No recent posts available from Membit.
          </div>
        )}
