...
export default function TrendingClusters() {
    ...
    async function load() {
      ...
      try {
        ...
        try {
          ...
          setError("Invalid JSON from clusters endpoint");
          setClusters([]);
          return;
        }
        if (!mounted) return;
  ...
  return (
    <div className="rounded-2xl border border-cyan-400/30 bg-[#0f0f0f]/80 neon-border p-4">
      <div className="px-1 py-2 text-sm tracking-wide text-foreground/70">
        Trending Clusters & Posts
      </div>
      <div className="mt-3 space-y-3">
        {loading && (
          <div className="text-xs text-foreground/60">Loading clusters…</div>
        )}
        ...
        {clusters &&
          clusters.map((c, idx) => (
            <div
              key={idx}
              className="p-3 rounded-md bg-[#061216] border border-cyan-400/10"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="text-sm font-semibold text-foreground">
                    {c.label ?? c.name ?? "Untitled Cluster"}
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-2">
                    {Array.isArray(c.posts) && c.posts.length > 0 ? (
                      c.posts.slice(0, 3).map((p: any, i: number) => (
                        <div
                          key={i}
                          className="p-2 rounded-md bg-[#071722] border border-cyan-400/10"
                        >
                          <div className="text-xs font-semibold text-foreground">
                            {p.title ?? p.content ?? p.text ?? p.uuid ?? p.id}
                          </div>
                          ...
                          <div className="text-[11px] text-foreground/70 mt-2">
                            ...
                            {p.sentiment !== undefined && p.sentiment !== null
                              ? Math.round(((p.sentiment + 1) / 2) * 100) + "%"
                              : p.engagement_score
                ...
                <div className="w-36 text-right">
                  ...
                  <div className="text-xs text-foreground/70 mt-2">
                    Engagement:{" "}
                    {c.engagement_score ? Math.round(c.engagement_score) : "—"}
                  </div>
                </div>
              </div>
            </div>
