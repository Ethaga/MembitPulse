...
export const flowiseChat: RequestHandler = async (req, res) => {
  try {
    ...
    if (!base) return res.status(500).json({ error: "FLOWISE_API_URL not configured on server" });
    ...
    const endpoint = baseUrl.match(/\/prediction|\/chat/) ? baseUrl : `${baseUrl}/prediction`;
